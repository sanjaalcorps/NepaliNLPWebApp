...
*/
        ...
        getNepaliRootWord("à¤µà¤¿à¤¸à¥�à¤®à¤¯à¤¾à¤¦à¤¿à¤¬à¥‹à¤§");
    ...
    public static String cleanWordToken(String word) {
        ...
        word = word.replaceAll("[\tà¥¦à¥¦â€“-â€”-â€¦_a-zA-Z0-9@#&\\$%:,;=->~â€œâ€�â€˜â€™!à¥¤ ||/\\+\\^\\*\\'\"\\.`\\(\\)\\[\\]\\{\\}\\.\\?\\\\]*", "");
        word = word.replaceAll("[à¥¦-à¥¯]", ""); //remove nepali numbers
        word = word.replaceAll("Â¥", "à¤°à¥�");
        word = word.replaceAll("Â©", "");
        word = word.replaceAll(">>", "");
        //word = word.replaceAll("", replacement)
        
		if ("Ã·".equalsIgnoreCase(word)) {
			word = "";
		} else if (word.indexOf("Ã·") == 0) {
			word = word.substring(1);//e.g. Ã·à¤…à¤¨à¥Œà¤ªà¤šà¤¾à¤°à¤¿à¤• > à¤…à¤¨à¥Œà¤ªà¤šà¤¾à¤°à¤¿à¤• 
		} else {
			word = word.replaceAll("Ã·", "/"); //e.g. à¤”à¤ªà¤šà¤¾à¤°à¤¿à¤•Ã·à¤…à¤¨à¥Œà¤ªà¤šà¤¾à¤°à¤¿à¤• > à¤”à¤ªà¤šà¤¾à¤°à¤¿à¤•/à¤…à¤¨à¥Œà¤ªà¤šà¤¾à¤°à¤¿à¤•
    ...
    public static String cleanSentence(String sentence) {
        ...
        sentence = sentence.replaceAll(">>", "");
        //sentence = sentence.replaceAll("[ a-zA-Z0-9]{1,}[\\(\\)\\+\\â€“\\*â€™,-\\.%;:=]{0,}", " ");//TEST
        sentence = sentence.trim().replaceAll("( )+", " ");//remove multiple spaces
        sentence = sentence.trim();
	...
	public static String cleanTitles(String title) {
		...
		title = title.replaceAll("[à¥¦à¥¦â€“-â€”-â€¦_a-zA-Z0-9@#&\\$%||/\\+\\^\\*\\.]*", "");
		title=title.trim();
		return title;
	}
